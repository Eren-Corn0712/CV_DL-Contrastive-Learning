...
from networks.res_cct import SupConBackbone, LinearClassifier
...
class SimCLRFTransform:
    def __init__(self, opt, eval_transform: bool = False, ) -> None:
        ...
        self.normalize = transforms.Normalize(mean=opt.mean,
                                              std=opt.std)
        ...
        if self.normalize is None:
            ...
            # TODO: Check Normalize affect train result ?
...
def set_backbone(opt):
    model = SupConBackbone(name=opt.model)
...
def train_backbone(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    process_bar.set_description(f'Training Backbone:{opt.model}')
    for idx, (images, labels) in enumerate(process_bar):
        ...
        process_bar.set_postfix_str(
            'epoch {epoch:} loss {loss.val:.4f} ({loss.avg:.4f})'.format(
...
def train_linear(train_loader, model, classifier, criterion, optimizer, epoch, opt):
    ...
    process_bar = tqdm(train_loader, total=len(train_loader), ascii=True, position=0, leave=True)
    process_bar.set_description(f'Training Cls:{opt.model}')
    for idx, (images, labels) in enumerate(process_bar):
        ...
        process_bar.set_postfix_str(
            'epoch {epoch:} loss {loss.val:.3f} ({loss.avg:.3f}) Acc@1 {top1.val:.3f} ({top1.avg:.3f})'.format(
                epoch=epoch, loss=losses, top1=top1))
...
def validation_backbone(val_loader, model, criterion, opt):
    ...
    with torch.no_grad():
        ...
        process_bar.set_description(f'Validate Backbone:{opt.model}')
        for idx, (images, labels) in enumerate(process_bar):
...
def validate_linear(val_loader, model, classifier, criterion, opt):
    ...
    with torch.no_grad():
        process_bar = tqdm(val_loader, total=len(val_loader), ascii=True, position=0, leave=True)
        process_bar.set_description(f'Validate Cls:{opt.model}')
        for idx, (images, labels) in enumerate(process_bar):
...
def main():
    ...
    # wandb
    your_api_key = '9a425f35becacacc9cd4ea2ed5563c6a1d03d96b'
    wandb.login(key=your_api_key)
    wandb_log = wandb.init(dir=opt_bk.wandb_folder,
                           config=vars(opt_bk),
                           project=project,
                           name=f'B_{opt_bk.model}_{opt_bk.method}_{exp_time}',
                           entity="Corn Chang",
                           )
    ...
    wandb_log = wandb.init(dir=opt_linear.wandb_folder,
                           ...
                           project=project,
                           name=f'L_{opt_linear.model}_{opt_linear.method}_{exp_time}'
                           )
